...
double yearly_interest(double balance) {
    ...
    double interestR = interest_rate(balance);
    return balance - (balance * (1 - (interestR/100.0)));
}
...
int years_until_desired_balance(double balance, double target_balance) {
    ...
    while(newBalance < target_balance) {
        newBalance = annual_balance_update(newBalance);
        nbYears++;
    }
